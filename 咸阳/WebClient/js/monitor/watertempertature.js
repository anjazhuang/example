...
$(function () {
    ...
    $("#bxlevel").select2().val(['1', '2', '3', '4']).trigger('change');
    initTreedata(6, $("#stcode").val(), $("#bxlevel").val(), $("#cztp").val())

    $("#stcode").on("select2:select", function (evt) {
        //这里是选中触发的事件
        //evt.params.data 是选中项的信息
        //alert(evt.params.data.id);
        initTreedata(6, evt.params.data.id, $("#bxlevel").val(), $("#cztp").val())
    });
    ...
    $("#bxlevel").on("change", function (evt) {
        if ($(this).val() !== null) {
            if ($(this).val().length > 5)
                initTreedata(6, $("#stcode").val(), "", $("#cztp").val())
            else
                initTreedata(6, $("#stcode").val(), $(this).val(), $("#cztp").val())
        } else {
            initTreedata(6, $("#stcode").val(), "", $("#cztp").val())
        }
    });
    $("#cztp").on("select2:select", function (evt) {
        ...
        //evt.params.data 是选中项的信息
        //alert(evt.params.data.id);
        initTreedata(6, $("#stcode").val(), $("#bxlevel").val(), evt.params.data.id)
    });
    //右边图表自适应方法
    ...
    $.ajax({
        ...
        if (data.success) {
            ...
            $.each(data.data, function (key, obj) {
                x_soil_data.push(obj.TM.replace("T", " "))
            });
            //水温图表option
            ...
            var intervalSandHtml = "<div class='tabletitle'>水温表</div>" +
                "<div><div class='btime'><strong>检索时间: </strong>" + $("#s-date input").val() + " 至 " + $("#e-date input").val() + "</div>" +
                "<div class='bunits'><span class='total'>水温：℃ 总记录数：" + data.data.length + "</span></div></div>";
            ...
            if (data.data.length > 0) {
                $.each(data.data, function (key, obj) {
                    var wtmp = obj.WTMP;
                    if (obj.STCD == "81004100" && obj.WTMP != null && obj.WTMP != "") {
                        wtmp = obj.WTMP + 0.5;
                    }
                    intervalSandHtml += '<tr>' +
                        '<td>' + (key + 1) + '</td>' +
...
var aclick = function (aa) {
    if ($('#sandtable tr:last').find('td').text() !== "无数据") {
        ...
        if (params.querytp == 0) {
            $(aa).attr("download", "水温表.xls");
            //alert(params.querytp+jst+total);
            exportExcel(aa, 'sandtable', '水温表', jst, total, time, company, 1)
        }

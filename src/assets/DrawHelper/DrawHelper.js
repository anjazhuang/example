  ...
  _.prototype.registerEditableShape = function (surface) {
    ...
    setListener(surface, 'mouseMove', function (position) {
      ...
      if (!surface._editMode) {
        _self._tooltip.showAt(new Cesium.Cartesian2(position.x, position.y + 50), "点击编辑图形");
      }
    });
...
//
    ...
    if (callbacks) {
      ...
      if (callbacks.tooltip) {
        setListener(billboard, 'mouseMove', function (position) {
          _self._drawHelper._tooltip.showAt(new Cesium.Cartesian2(position.x, position.y + 50), callbacks.tooltip());
        });
    ...
    mouseHandler.setInputAction(function (movement) {
      ...
      if (position != null) {
        ...
        if (cartesian) {
          ...
          tooltip.showAt(new Cesium.Cartesian2(position.x, position.y + 50), "<p>添加点功能：点击添加标注 </p>");
        } else {
          tooltip.showAt(new Cesium.Cartesian2(position.x, position.y + 50), "<p>请点击球体进行标注添加</p>");
        }
      }
    }, Cesium.ScreenSpaceEventType.MOUSE_MOVE);
  ...
  _.prototype.startDrawingPolyshape = function (isPolygon, options) {
    ...
    mouseHandler.setInputAction(function (movement) {
      ...
      if (position != null) {
        if (positions.length == 0) {
          tooltip.showAt(new Cesium.Cartesian2(position.x, position.y + 50), "<p>添加第一个点</p>");
        } else {
          var cartesian = scene.camera.pickEllipsoid(position, ellipsoid);
          if (cartesian) {
            positions.pop();
            ...
            // show tooltip
            tooltip.showAt(new Cesium.Cartesian2(position.x, position.y + 50), "<p>添加一个新的坐标点 (" + positions.length + ")</p>" + (positions.length > minPoints ? "<p>右键完成多边形绘制</p>" : ""));
          }
        }
      }
    }, Cesium.ScreenSpaceEventType.MOUSE_MOVE);
  ...
  _.prototype.startDrawingExtent = function (options) {
    ...
    mouseHandler.setInputAction(function (movement) {
      ...
      if (position != null) {
        if (extent == null) {
          tooltip.showAt(new Cesium.Cartesian2(position.x, position.y + 50), "<p>点击添加矩形坐标点</p>");
        } else {
          var cartesian = scene.camera.pickEllipsoid(position, ellipsoid);
          if (cartesian) {
            var value = getExtent(firstPoint, ellipsoid.cartesianToCartographic(cartesian));
            updateExtent(value);
            tooltip.showAt(new Cesium.Cartesian2(position.x, position.y + 50), "<p>拖动改变矩形大小</p><p>点击完成矩形绘制</p>");
          }
        }
      }
    }, Cesium.ScreenSpaceEventType.MOUSE_MOVE);
  ...
  _.prototype.startDrawingCircle = function (options) {
    ...
    mouseHandler.setInputAction(function (movement) {
      ...
      if (position != null) {
        if (circle == null) {
          tooltip.showAt(new Cesium.Cartesian2(position.x, position.y + 50), "<p>添加圆心点</p>");
        } else {
          var cartesian = scene.camera.pickEllipsoid(position, ellipsoid);
          if (cartesian) {
            circle.setRadius(Cesium.Cartesian3.distance(circle.getCenter(), cartesian));
            markers.updateBillboardsPositions(cartesian);
            tooltip.showAt(new Cesium.Cartesian2(position.x, position.y + 50), "<p>移动鼠标改变圆半径</p><p>点击完成绘制圆</p>");
          }
        }
      }
    }, Cesium.ScreenSpaceEventType.MOUSE_MOVE);
  ...
  // 注册绘制事件
  _.prototype.registerDrawEvent = function (type, callback) {
    let _self = this
    ...
    if (type === 'marker') {
      ...
      this.addListener('markerCreated', function (event) {
        ...
        var primitiveItem = scene.primitives.add(b);
        let billboard = b.add({
          show: true,
        ...
        _self._primitivesManagre.marker.push(primitiveItem);
        _self._primitivesManagre.allPrimitives.push(primitiveItem);
    ...
    // 线标绘事件
      ...
      this.addListener('polylineCreated', function (event) {
        ...
        var primitiveItem = scene.primitives.add(polyline);
        polyline.setEditable();
        _self._primitivesManagre.polyline.push(polyline);
        _self._primitivesManagre.allPrimitives.push(polyline);
    ...
    // 面标绘事件
      ...
      this.addListener('polygonCreated', function (event) {
        ...
        var primitiveItem = scene.primitives.add(polygon);
        _self._primitivesManagre.polygon.push(polygon);
        _self._primitivesManagre.allPrimitives.push(polygon);
        polygon.setEditable();
    ...
    // 清理所有标绘事件
    else if (type === 'cleanAll') {
      for (var i in _self._primitivesManagre.allPrimitives) {
        scene.primitives.remove(_self._primitivesManagre.allPrimitives[i])

      }
      _self._primitivesManagre.allPrimitives.splice(0, _self._primitivesManagre.allPrimitives.length);

      for (var i = 0; i < scene.primitives._primitives.length; i++) {
        var str = scene.primitives._primitives[i].constructor + ''
        ...
        if (p_type_name === 'BillboardCollection') {
          var billboardCollection = scene.primitives._primitives[i]
          if (billboardCollection) {
            for (var j = 0; j < billboardCollection._billboards.length; j++) {
              if (billboardCollection._billboards[j]._id && billboardCollection._billboards[j]._id.indexOf('drawPoint') != -1) {
                scene.primitives._primitives[i].remove(billboardCollection._billboards[j]);
              }
            }
          }
        }
    ...
    // 标绘回撤事件
      ...
      var str = _self._primitivesManagre.allPrimitives[_self._primitivesManagre.allPrimitives.length - 1].constructor + ''
      var start = str.replace("function", "|").indexOf('|')
      var p_type_name = ''
      if (start != -1) {
        var end = str.replace("function", "|").indexOf("(")
      ...
      // 回撤过程中需要清除标绘图层管理类中对应类型数组中的primitiveItem
      if (p_type_name === 'BillboardCollection') {
        _self._primitivesManagre.marker.splice(_self._primitivesManagre.marker.length - 1, 1)
      } else {
        if (_self._primitivesManagre.allPrimitives[_self._primitivesManagre.allPrimitives.length - 1].primitiveType === 'polyline') {
          _self._primitivesManagre.polyline.splice(_self._primitivesManagre.polyline.length - 1, 1)
        } else if (_self._primitivesManagre.allPrimitives[_self._primitivesManagre.allPrimitives.length - 1].primitiveType === 'polygon') {
          _self._primitivesManagre.polygon.splice(_self._primitivesManagre.polygon.length - 1, 1)
        }
      }
      // 回撤需要清除标绘图层管理类中所有primitive中的最后一个
      scene.primitives.remove(_self._primitivesManagre.allPrimitives[_self._primitivesManagre.allPrimitives.length - 1])
    }
  }
  ...
  _.prototype.exportLayer = function () {
    ...
    if (markerArr.length == 1) {
      jsonObj.push({ "type": "Point", "coordinates": markerArr[0] });
    }
    // 多点标注图层
    if (markerArr.length > 1) {
    ...
    // 单线标注图层
    if (lineArr.length == 1) {
      jsonObj.push({ "type": "LineString", "coordinates": lineArr[0] });
    }
    // 多线标注图层
    if (lineArr.length > 1) {
    ...
    if (polygonArr.length == 1) {
      jsonObj.push({ "type": "Polygon", "coordinates": polygonArr[0] });
    }
    // 多面标注图层
    if (polygonArr.length > 1) {
  ...
  // _.prototype.addToolbar = function() {
  //     return new _.DrawHelperEvent(this);
  // }

import styles from './ImportRSTableModal.less';
...
export default class ImportRSTableModal extends Component {
  state = {
    visible: false,
    fileList: [],//上传文件
    timephaseid:0,//时相
  ...
  importRSTableModalClose = () => {
    const { dispatch } = this.props;
    dispatch({
      type: 'indexPage/updateState',
      payload: {
        importRSTableModalShow: false
      }
    });
  }
    ...
    return (
      <LocaleProvider locale={zh_CN}>
        <Modal
          ...
          onCancel={this.importRSTableModalClose}
          style={{ top: 200 }}
          footer={[<Button className={styles.bigbtn} onClick={this.Views}>加载预览</Button>, <Button className={styles.bigbtn}>导入数据库</Button>]}
          maxable={false}
          minable={false}
          width={500}>
          <div>
